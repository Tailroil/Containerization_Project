...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${import.meta.env.VITE_API_USER_SERVICE}/me`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((response) => {
        // Conversion explicite du type pour correspondre à { email: string }
        setUser(response.data as { email: string });
      })
      .catch(() => {
        localStorage.removeItem("token");
        navigate("/");
      })
      // Utilisation d'un second then à la place de finally
      .then(() => {
        setLoading(false);
      });
  }, [navigate]);
  ...
  const calculateConsumption = () => {
    ...
    if (selectedCountry) {
      axios
        ...
        .then((response) => {
          // Forcer le type en string
          const dataStr = response.data as string;
          const emissionFactor = parseFloat(dataStr);
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        const countriesList = (response.data as string)
          .split("\n")
